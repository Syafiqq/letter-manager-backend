        ...
        if ($request->expectsJson())
        {
            ...
            switch (get_class($exception))
            {
                case ModelNotFoundException::class :
                    return response()
                        ->json(PopoMapper::alertResponse(HttpStatus::NOT_FOUND, 'Resource Not Found')
                            ->serialize(), HttpStatus::NOT_FOUND);
                case ValidationException::class :
                    /** @var ValidationException $exception */
                    return response()
                        ->json(PopoMapper::jsonResponse($statusCode, strlen($exception->getMessage()) < 1 ? 'Invalid Data' : $exception->getMessage(), $exception->errors(), [], ['Invalid form request'])
                            ->withLevel(ResponseKind::DANGER)
                            ->serialize(), $statusCode);
                case AuthorizationException::class :
                    /** @var AuthorizationException $exception */
                    return response()
                        ->json(PopoMapper::alertResponse(HttpStatus::NOT_FOUND, strlen($exception->getMessage()) < 1 ? 'Unknown Request' : $exception->getMessage())
                            ->withAlertLevel(ResponseKind::DANGER)
                            ->serialize(), HttpStatus::FORBIDDEN);
                default :
                    return response()
                        ->json(PopoMapper::alertResponse($statusCode, strlen($exception->getMessage()) < 1 ? 'Unknown Request' : $exception->getMessage())
                            ->withAlertLevel(ResponseKind::DANGER)
                            ->serialize(), $statusCode);
            }
        }
